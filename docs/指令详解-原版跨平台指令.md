...
| [`Get-Verb`](#get-verb) | Microsoft.PowerShell.Utility | 仅7 | 仅7提供 | 获取已批准的 PowerShell 谓词。 | 未实现 | 模块与程序集（不适用单文件解释器） |
...
| [`Import-PowerShellDataFile`](#import-powershelldatafile) | Microsoft.PowerShell.Utility | 仅7 | 仅7提供 | 从 .psd1 文件中导入值，而无需调用其内容。 | 未实现 | 模块与程序集（不适用单文件解释器） |
...
| [`Invoke-Item`](#invoke-item) | Microsoft.PowerShell.Management | 都有 | 语法不同 | 对指定项执行默认作。 | Go实现 | 目前只打印路径，不真正打开。 |
...
| [`Join-String`](#join-string) | Microsoft.PowerShell.Utility | 仅7 | 仅7提供 | 将管道中的对象合并为单个字符串。 | Go实现 | 不写 `-Separator` 时默认用空格连接（PowerShell 默认无分隔符直接相连）。 |
...
- 与原版差异：路径不存在时静默创建一个空文件（PowerShell 会报“找不到路径”错误）。
...
- 与原版差异：内存字段名为 Memory（PowerShell 名为 WS），值为物理内存字节数，与 PowerShell 的 WS 语义一致；`-Name` 按子串匹配（PowerShell 用精确名或通配符）。
...
| `Id` | 进程号（int） |
| `ProcessName` | 进程名 |
| `CPU` | 累计 CPU 时间，秒（与 PowerShell 的 CPU(s) 同语义）。 |
| `Memory` | 物理内存字节数（与 PowerShell 的 WS 语义一致）。 |
...
出处：[官方中文文档（7）](https://learn.microsoft.com/zh-cn/powershell/module/microsoft.powershell.utility/get-verb?view=powershell-7.5)
...
出处：[官方中文文档（7）](https://learn.microsoft.com/zh-cn/powershell/module/microsoft.powershell.utility/import-powershelldatafile?view=powershell-7.5)
...
- 与原版差异：不写 `-Separator` 时默认用空格连接（PowerShell 默认无分隔符直接相连）。
...
- 行为：目录不存在或不是目录 → 报错写 stderr、$?=false。成功后切换当前目录。
