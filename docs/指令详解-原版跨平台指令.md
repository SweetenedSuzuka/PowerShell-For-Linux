...
| [`Get-ItemPropertyValue`](#get-itempropertyvalue) | Microsoft.PowerShell.Management | 都有 | 语法不同 | 获取指定项的一个或多个属性的值。 | Go实现 | 只取单个属性；缺 -Name 时返回空。 |
...
- 类型：Go实现。
- 功能：按名取单个属性值。对应 bash 按字段取 `stat` 输出，如 `stat -c %s`。

| 参数 | 类型 | 含义 |
| :--- | :--- | :--- |
| `-Path`（位置 0） | path | 目标路径 |
| `-Name`（位置 1） | string | 要取的属性名，如 `Get-ItemPropertyValue x -Name Length` |
...
- 输出：随机整数或取样对象。
- 行为：最小值大于等于最大值、个数小于 1、类型转不成整数、参数集冲突逐个报错继续。
...
- 行为：-Add 与 -Remove 同用时先删后加；属性缺失、非集合、空输入项逐个报错继续；相等按字符串大小写折叠。
