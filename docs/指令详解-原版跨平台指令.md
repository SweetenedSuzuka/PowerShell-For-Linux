...
| [`Clear-ItemProperty`](#clear-itemproperty) | Microsoft.PowerShell.Management | 都有 | 语法不同 | 清除属性的值，但不删除该属性。 | 未实现 | 含 Windows 专属内容（注册表 / 服务 / 回收站 / 补丁 / 事件） |
...
| [`Copy-ItemProperty`](#copy-itemproperty) | Microsoft.PowerShell.Management | 都有 | 语法不同 | 将属性和值从指定位置复制到另一个位置。 | 未实现 | 含 Windows 专属内容（注册表 / 服务 / 回收站 / 补丁 / 事件） |
...
| [`Export-Csv`](#export-csv) | Microsoft.PowerShell.Utility | 都有 | 语法不同 | 将对象转换为一系列字符分隔值（CSV）字符串，并将字符串保存到文件中。 | Go实现 | 文本与 ConvertTo-Csv 一致；-Property 为扩展参数；缺输入不建文件。 |
...
| [`Get-Credential`](#get-credential) | Microsoft.PowerShell.Security | 都有 | 语法不同 | 获取基于用户名和密码的凭据对象。 | Go实现 | 密码回显；表格只显示用户名；无凭据方法。 |
...
| [`Get-Error`](#get-error) | Microsoft.PowerShell.Utility | 仅7 | 仅7提供 | 获取并显示当前会话中的最新错误消息。 | Go实现 | 记录只有消息文本；条数不得小于 1。 |
...
| [`Get-ItemPropertyValue`](#get-itempropertyvalue) | Microsoft.PowerShell.Management | 都有 | 语法不同 | 获取指定项的一个或多个属性的值。 | Go实现 | 只取出单个属性；缺 -Name 时返回空。 |
...
| [`Get-SecureRandom`](#get-securerandom) | Microsoft.PowerShell.Utility | 仅7 | 仅7提供 | 获取随机数，或从集合中随机选择对象。 | Go实现 | 端点只支持整数；无种子参数。 |
...
| [`Import-Csv`](#import-csv) | Microsoft.PowerShell.Utility | 都有 | 语法不同 | 从字符分隔值 （CSV） 文件中的项创建类似表的自定义对象。 | Go实现 | 首行类型行跳过；缺路径返回空。 |
...
| [`Measure-Object`](#measure-object) | Microsoft.PowerShell.Utility | 都有 | 简介不同；语法不同 | 5.1：计算对象的数值属性，以及字符串对象（如文本文件）中的字符、字词和行。 / 7：计算对象的数值属性，以及字符串对象（如文本文件）中的字符、单词和行。 | Go实现 | Min/Max 对混合数字与非数字的输入只统计数字（PowerShell 把非数字当字符串参与比较）；遇非数字不报错只置为空（PowerShell 报非终止错误）。 |
| [`Move-Item`](#move-item) | Microsoft.PowerShell.Management | 都有 | 语法不同 | 将项从一个位置移动到另一个位置。 | Go实现 | 目录移动需加 `-Recurse`。 |
| [`Move-ItemProperty`](#move-itemproperty) | Microsoft.PowerShell.Management | 都有 | 简介不同；语法不同 | 5.1：将属性从一个位置移到另一个位置。 / 7：将属性从一个位置移动到另一个位置。 | 未实现 | 含 Windows 专属内容（注册表 / 服务 / 回收站 / 补丁 / 事件） |
...
| [`New-ItemProperty`](#new-itemproperty) | Microsoft.PowerShell.Management | 都有 | 语法不同 | 为项创建新属性并设置其值。 | 未实现 | 含 Windows 专属内容（注册表 / 服务 / 回收站 / 补丁 / 事件） |
...
| [`Out-Default`](#out-default) | Microsoft.PowerShell.Core | 都有 | 简介不同 | 5.1：将输出发送到默认格式化程序，并将输出发送到默认输出 cmdlet。 / 7：将输出发送到默认的格式化程序和默认的输出 cmdlet。 | Go实现 | -Transcript 接受忽略。 |
...
| [`Remove-ItemProperty`](#remove-itemproperty) | Microsoft.PowerShell.Management | 都有 | 简介不同；语法不同 | 5.1：从项中删除属性及其值。 / 7：删除某个项目中的属性及其值。 | 未实现 | 含 Windows 专属内容（注册表 / 服务 / 回收站 / 补丁 / 事件） |
...
| [`Rename-ItemProperty`](#rename-itemproperty) | Microsoft.PowerShell.Management | 都有 | 简介不同；语法不同 | 5.1：重命名项的属性。 / 7：将某一项的属性重命名。 | 未实现 | 含 Windows 专属内容（注册表 / 服务 / 回收站 / 补丁 / 事件） |
...
| [`Set-StrictMode`](#set-strictmode) | Microsoft.PowerShell.Core | 都有 | 无 | 在表达式、脚本和脚本块中建立并强制实施编码规则。 | Go实现 | 只做未定义变量检查；各版本行为一致。 |
...
| [`Start-Transcript`](#start-transcript) | Microsoft.PowerShell.Host | 都有 | 语法不同 | 创建指向文本文件的 PowerShell 会话的所有或部分的记录。 | Go实现 | 默认家目录命名；重复启动先结束旧的再开新的。 |
...
| [`Stop-Transcript`](#stop-transcript) | Microsoft.PowerShell.Host | 都有 | 语法不同 | 停止记录。 | Go实现 | 写结束块并关闭文件；未记录时报错。 |
| [`Switch-Process`](#switch-process) | Microsoft.PowerShell.Core | 仅7 | 仅 Linux/macOS 版 7 提供（Windows 版未收录） | 在 Linux 和 macOS 上，cmdlet 调用 execv() 函数以提供与 POSIX shell 类似的行为。 | Go实现 | 首个为可执行文件；未给命令直接返回。 |
...
| [`Update-List`](#update-list) | Microsoft.PowerShell.Utility | 都有 | 无 | 向包含对象集合的属性值中添加项并删除项。 | Go实现 | 数组一律可改；缺 -Property 返回空。 |
...
- 行为：New 时变量已存在且无 -Force → 报错；Set/New 时只读自动变量（PID 等）赋值 → 报错，Clear 时静默无操作。Remove 直接删除 map；Clear 置为 $null。
...
| `-CaseSensitive` | switch | 大小写敏感比较；默认不敏感 |
| `-IncludeEqual` | switch | 输出相等项（`==`），默认只输出差异 |
...
- 与原版差异：文本格式与 ConvertTo-Csv 一致；-Property 筛选列是本程序扩展，原版没有该参数；缺输入时不建文件也不报错；-Path 只接受单个路径。
...
| 参数 | 类型 | 含义 |
| :--- | :--- | :--- |
| `-Path`（位置 0） | path | 目标文件路径 |
| `-InputObject` | object | 要写的数据（一般经管道输入） |
| `-Property` | string[] | 只写这些列（本程序扩展） |
| `-Delimiter` | string | 列分隔符，单个字符，默认逗号 |
| `-Append` | switch | 追加到已有非空文件，不重复表头 |
| `-NoClobber` | switch | 文件已存在时报错，不覆盖 |
| `-NoTypeInformation` | switch | 直接接受，不写类型行（默认行为） |
| `-Encoding` | string | 写文件编码，默认无字节序标记 |
...
- 实现：按首对象属性（或无表格定义对象时按首对象 Props）生成列，右对齐数值列，表头加 `----` 下划线。标量（字符串/数字/布尔/脚本块）无可制表属性，直接逐行显示。
- DateTime 对象用默认列（DisplayHint、Date、Day、DayOfWeek、DayOfYear、Hour、Kind、Millisecond、Microsecond、Nanosecond、Minute、Month、Second、Ticks、TimeOfDay、Year，顺序与 PowerShell 一致）。
- 自定义对象流合成一张表处理，连续同形对象共用首对象列，后出属性不另起列，如缺失则置为空。单个对象使用列表处理。
...
- 与原版差异：密码输入回显（原版屏蔽）；输出表格只显示用户名；无凭据方法（如 GetNetworkCredential）；非交互运行返回空（Read-Host 此时报错）。

- 类型：Go实现。
- 功能：终端提示输入用户名和密码，得到凭据对象。

| 参数 | 类型 | 含义 |
| :--- | :--- | :--- |
| `-UserName`（位置 0） | string | 用户名，给出时只提示输入密码；位置实参是凭据对象时按 -Credential 处理 |
| `-Credential` | object | 已有凭据，单独给出时直接返回 |
| `-Message` | string | 提示消息 |
| `-Title` | string | 提示标题 |

- 输出：PSCredential 对象，字段 UserName、Password；无输入（流结束）与非交互运行时无输出。
- 行为：标题或消息给出时显示它们，不显示默认两行；管道输入忽略，照常提示；超量位置实参报错；-Credential 与用户名、消息、标题同时给出时报错。
...
| 参数 | 类型 | 含义 |
| :--- | :--- | :--- |
| `-Date`（位置 0） | string | 指定日期时间（不写时区按本地时区解析），支持 `2006-01-02 15:04:05`、`2006-01-02T15:04:05`、`2006-01-02`、`2006-1-2 15:04:05`、`2006-1-2T15:04:05`、`2006-1-2`、`2006/1/2`、RFC3339 |
...
- 与原版差异：记录只有消息文本，无异常堆栈等明细；条数不得小于 1（为零与为负都报错）。
...
| 参数 | 类型 | 含义 |
| :--- | :--- | :--- |
| `-Newest`（位置 0） | int | 获取最新几条，默认 1 |
| `-InputObject` | object | 要展示的错误，只接受单个 |

- 输出：错误记录对象；无记录、无输入、数组输入时无输出。
- 行为：-Newest 与输入对象并存、管道与命名输入并存逐个报错继续。
...
- 输出：名字、语法（`命令 [-参数 <类型>]...`）、别名。无匹配时查询别名并显示其目标；都不是则提示找不到。
...
- 与原版差异：只支持文件路径的 5 个字段中的单个属性名；缺 -Name 时返回空而不报错；-Path 只接受单个路径。

- 类型：Go实现。
- 功能：按名取出单个属性值。对应 bash 按字段取出 `stat` 输出，如 `stat -c %s`。

| 参数 | 类型 | 含义 |
| :--- | :--- | :--- |
| `-Path`（位置 0） | path | 目标路径 |
| `-Name`（位置 1） | string | 要取出的属性名，如 `Get-ItemPropertyValue x -Name Length` |
...
| `-MemberType` | string | 成员类型过滤（Property / TypeName 等，大小写不敏感，默认返回全部） |
...
- 实现：Linux 读取 `/proc/<pid>/comm` 获取进程名、读取 `/proc/<pid>/stat` 获取 utime/stime/rss；其它平台回退为当前进程。
- 输出：Process 对象，字段：
...
- 与原版差异：端点只支持整数；无种子参数（随机源不可设种子）。

- 类型：Go实现。
- 功能：出安全随机数，或在给定范围/列表里随机取。

| 参数 | 类型 | 含义 |
| :--- | :--- | :--- |
| `-Maximum`（位置 0） | object | 上限（不含），整数；数组按取样处理，如 `Get-SecureRandom (1,2,3)` |
| `-Minimum` | int | 下限（含），默认 0 |
| `-InputObject` | object | 取样集合（一般经管道输入） |
| `-Count` | int | 取几个，默认 1，不得小于 1 |
| `-Shuffle` | switch | 打乱整个集合，不与 -Count 同用 |

- 输出：随机整数或取样对象。
- 行为：最小值大于等于最大值、个数小于 1、类型无法转换为整数、参数集冲突逐个报错继续。
...
- 与原版差异：读取 /etc/timezone。

- 类型：Go实现（读取 /etc/timezone）。
- 发行版：所有。
- 同组：Set-TimeZone。
- 功能：看当前时区。
- 参数：无。
...
- 实现：Linux 读取 /proc/uptime 秒数；其它平台为 0。
...
| `-CaseSensitive` | switch | 大小写敏感分组；默认不敏感 |

- 实现：按属性值（无则按对象字符串）分组，默认大小写不敏感（统一大小写后合并），Name 取组内首次出现的原值。
...
| 参数 | 类型 | 含义 |
| :--- | :--- | :--- |
| `-Path`（位置 0） | path | CSV 文件路径；可多个（`Import-Csv a.csv,b.csv` 逐个读出后合并） |
| `-Delimiter`（位置 1） | string | 列分隔符，单个字符，默认逗号 |
| `-Header` | string[] | 自定义列名；给出时文件首行也作数据 |

- 输出：每行一个表格对象，字段值都是字符串；空文件与只有表头的文件无输出。
- 行为：首行类型行跳过；缺路径返回空；路径不存在 → 报错、$?=false；超量位置实参报错。
...
| `-Property` | string | 取出该属性的值并转换为文本再连接 |
...
- 与原版差异：Min/Max 对混合数字与非数字的输入只统计数字（PowerShell 把非数字当字符串参与比较）；遇非数字不报错只置为空（PowerShell 报非终止错误）。
...
- 输出：MeasureInfo 对象，字段 Count、Average、Sum、Maximum、Minimum、StandardDeviation、Property 总是补全，未指定的统计为 $null。
- 行为：`-Property` 模式下 Count 只统计有该属性的对象。Sum/Average 遇非数字输入时该统计作废（置为 $null）；Min/Max 仅统计数字。
...
| 参数 | 类型 | 含义 |
| :--- | :--- | :--- |
| `-Transcript` | switch | 接受忽略 |
| `-InputObject` | object | 要显示的数据（一般经管道输入） |

- 输出：无输出对象；直接渲染到屏幕，下游收不到数据。
- 行为：管道与 -InputObject 并存报错；超量位置实参报错；无输入时无输出。
...
| `-Force` | switch | 强制删除，对应 `rm -f`（文件删除同样调用 RemoveAll） |
...
| `-ExpandProperty` | string | 取该属性值本身输出，不做对象包装；值为数组时展开（如 `Select-Object -ExpandProperty Name`） |
...
| `-CaseSensitive` | switch | 大小写敏感匹配；默认不敏感（对应 `grep -i`） |
...
- 与原版差异：各版本行为一致，只做未定义变量检查。

- 类型：Go实现。
- 功能：打开或关闭严格模式，打开后读未定义变量报错。

| 参数 | 类型 | 含义 |
| :--- | :--- | :--- |
| `-Version`（位置 0） | string | 版本号，1.0、2.0、3.0、Latest 及其一位写法，如 `Set-StrictMode -Version Latest` |
| `-Off` | switch | 关掉严格模式 |

- 输出：无输出。
- 行为：错误跳过本句继续执行，可被 try 捕获；函数内继承，外层不受影响；自动变量与环境变量不触发。
...
| `-CaseSensitive` | switch | 大小写敏感；默认不敏感 |

- 实现：稳定排序；数值比大小、字符串大小写不敏感比较。`-Unique` 默认按排序键统一大小写后去重，`-CaseSensitive` 时按原值去重。
...
- 与原版差异：-Force 与调用信息头部开关接受忽略；-LiteralPath 与 -Path 相同；不支持 -OutputDirectory；记录中途的重定向输出不进文件。

- 类型：Go实现。
- 功能：开始会话记录，主机输出同时写入文本文件。

| 参数 | 类型 | 含义 |
| :--- | :--- | :--- |
| `-Path`（位置 0） | path | 记录文件路径，未给出时按家目录默认命名，如 `Start-Transcript session.log` |
| `-LiteralPath` | path | 与 -Path 相同 |
| `-Append` | switch | 追加到已有文件末尾 |
| `-Force` | switch | 接受忽略 |
| `-NoClobber` | switch | 文件已存在时报错，不覆盖 |
| `-IncludeInvocationHeader` | switch | 接受忽略 |
| `-UseMinimalHeader` | switch | 接受忽略 |

- 输出：提示字符串（记录已启动，输出文件为某路径）。
- 行为：重复启动先结束旧的再开新的，不报错；缺少路径时按默认命名。
...
- 类型：Go实现。
- 功能：停止会话记录，写结束块并关闭记录文件。

- 输出：提示字符串（记录已停止，输出文件为某路径）。
- 行为：未记录时报错；空闲时加 -WhatIf 只打印预演行，不报错。
...
- 类型：Go实现。
- 功能：用给定命令替换当前进程（execv 语义）。

| 参数 | 类型 | 含义 |
| :--- | :--- | :--- |
| `-WithCommand`（位置 0） | string[] | 首个为可执行文件，其余为实参，如 `Switch-Process /bin/echo hello` |

- 输出：无（成功时进程已被替换，不返回）。
- 行为：未给命令直接成功返回；命令不存在报错；-WhatIf/-Confirm 不接受，传入报错。
...
| `-PathType` | string | 类型过滤：`Leaf` 只认文件、`Container` 只认目录、`Any`/默认不限制 |
...
- 与原版差异：本程序数组一律可改，原版定长数组报错；缺 -Property 时返回空而不报错。
...
| 参数 | 类型 | 含义 |
| :--- | :--- | :--- |
| `-Property`（位置 0） | string | 要改的集合属性名，如 `Update-List -Property L -Add 4` |
| `-Add` | object | 追加这些项 |
| `-Remove` | object | 删掉首个相等的项，找不到不报错 |
| `-Replace` | object | 整列换成这些项，不与 -Add/-Remove 同用 |
| `-InputObject` | object | 要改的对象（一般经管道输入） |

- 输出：改写后的输入对象；无输入与缺 -Property 时无输出。
- 行为：-Add 与 -Remove 同用时先删后加；属性缺失、非集合、空输入项逐个报错继续；相等按字符串统一大小写。
...
- 实现：Linux 读 `/proc/<pid>/stat` 判定进程是否活动（Z/X 视为已结束），每 100ms 轮询；按名轮询进程列表直到消失。对入口已不存在的目标直接报错。
