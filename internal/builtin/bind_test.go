...
// bindViaEval 通过 eval → builtin 的完整接口路径执行参数绑定。
func bindViaEval(t *testing.T, src string) (*builtin.BoundArgs, error) {
	t.Helper()
...
// assertNamedArg 断言命名参数存在且字符串值相等。
func assertNamedArg(t *testing.T, args *builtin.BoundArgs, name, expected string) {
	t.Helper()
	v := args.Get(name)
	if v == nil {
	...
	if v.String() != expected {
		t.Fatalf("%s = %q，想要 %q", name, v.String(), expected)
	}
}

// TestBindPositionalMapping 位置实参按规格中心化映射到命名参数。
func TestBindPositionalMapping(t *testing.T) {
	args, err := bindViaEval(t, "Set-Content a.txt hello")
	if err != nil {
		t.Fatal(err)
	}
	assertNamedArg(t, args, "Path", "a.txt")
	assertNamedArg(t, args, "Value", "hello")
...
func TestBindUnsetSlotNotPositional(t *testing.T) {
	...
	assertNamedArg(t, args, "Path", "b.txt")
	assertNamedArg(t, args, "Value", "bval")
}

// TestBindSkipNamedSlot 已命名的槽位跳过，位置实参填入第一个空槽位。
func TestBindSkipNamedSlot(t *testing.T) {
	args, err := bindViaEval(t, "Join-Path -ChildPath child /tmp")
	if err != nil {
		t.Fatal(err)
	}
	assertNamedArg(t, args, "ChildPath", "child")
	assertNamedArg(t, args, "Path", "/tmp")
	if !args.PosMapped["Path"] {
...
func TestBindScriptBlockLazy(t *testing.T) {
	args, err := bindViaEval(t, "Where-Object { $_ -gt 2 }")
	if err != nil {
		t.Fatal(err)
	}
...
func TestBindExcessPositional(t *testing.T) {
	args, err := bindViaEval(t, "Copy-Item a b c")
	if err != nil {
		t.Fatal(err)
	}
	assertNamedArg(t, args, "Path", "a")
	assertNamedArg(t, args, "Destination", "b")
...
func TestBindPosMappedOnlyMapped(t *testing.T) {
	args, err := bindViaEval(t, "Set-Content -Path a -Value b")
	if err != nil {
		t.Fatal(err)
	}
...
func TestBindArrayPositional(t *testing.T) {
	args, err := bindViaEval(t, "Copy-Item a1.txt,a2.txt d")
	if err != nil {
		t.Fatal(err)
	}
	v := args.Get("Path")
...
func TestBindWhatIfRecorded(t *testing.T) {
	...
	args, err = bindViaEval(t, "Remove-Item a -Confirm")
	if err != nil {
		t.Fatal(err)
	}
	...
	args, err = bindViaEval(t, "Remove-Item a -WhatIf:$false")
	if err != nil {
		t.Fatal(err)
	}
	if args.Switch("WhatIf") {
...
func TestBindUnknownNamed(t *testing.T) {
	_, err := bindViaEval(t, "Set-Content -NoSuch x")
	if err == nil {
...
func TestBindValueObjects(t *testing.T) {
	args, err := bindViaEval(t, "Start-Sleep 2")
	if err != nil {
		t.Fatal(err)
	}
	v := args.Get("Seconds")
...
func TestBindInlineSwitch(t *testing.T) {
	...
	for _, tc := range cases {
		args, err := bindViaEval(t, tc.src)
		if err != nil {
			t.Fatalf("%q 绑定出错: %v", tc.src, err)
		}
		if args.Switches["Recurse"] != tc.expected {
			t.Errorf("%q 开关 = %v，想要 %v", tc.src, args.Switches["Recurse"], tc.expected)
		}
	}
}
